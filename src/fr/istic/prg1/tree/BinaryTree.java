...
public class BinaryTree<T> {
	...
	public class TreeIterator implements Iterator<T> {
		...
		public void goLeft() {
			...
			this.stack.push(currentNode); //on met le currentNode dans la pile de pères
			currentNode = currentNode.left; //on change le currentNode par le fils gauche de l'ancien
		...
		public void goRight() {
			...
			this.stack.push(currentNode); //on met le currentNode dans la pile de pères
			currentNode = currentNode.right; //on change le currentNode par le fils gauche de l'ancien
		}
		...
		public void goUp() {
			...
			currentNode = this.stack.pop(); //on récupère le dernier père qui est sur le stack, donc le père de currentNode
		}
		...
		public NodeType nodeType() {
			...
			if(!currentNode.left.isEmpty()) // Si gauche est fonctionnel
		    {
				if(!currentNode.right.isEmpty())// et si droite est fonctionnel
		    	{
					return NodeType.DOUBLE;		//alors c'est un double
		    	}
				return NodeType.SIMPLE_LEFT;	//sinon il n'y a que gauche
		    }
			else // Si gauche est vide
			{
				if(!currentNode.right.isEmpty()) // et que droite est fonctionnel
		    	{
					return NodeType.SIMPLE_RIGHT; // alors il n'y a que droite 
		    		
		    	}
				return NodeType.LEAF; //sinon il n'y a pas de fils donc c'est une feuille 
			}
		}
